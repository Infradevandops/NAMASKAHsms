...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('metadata', postgresql.JSONB(), nullable=True),
        sa.ForeignKeyConstraint(['template_id'], ['pricing_templates.id'], ondelete='SET NULL'),
    ...
    template_id = connection.execute(sa.text("SELECT id FROM pricing_templates WHERE name = 'Standard Pricing'")).scalar()
    ...
    template_id = connection.execute(sa.text("SELECT id FROM pricing_templates WHERE name = 'Holiday Sale'")).scalar()
    ...
    template_id = connection.execute(sa.text("SELECT id FROM pricing_templates WHERE name = 'EU Pricing'")).scalar()
    ...
    template_id = connection.execute(sa.text("SELECT id FROM pricing_templates WHERE name = 'Test Pricing'")).scalar()
