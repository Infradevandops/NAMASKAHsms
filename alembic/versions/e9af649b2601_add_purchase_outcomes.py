...
def upgrade() -> None:
    ...
    if not inspector.has_table("purchase_outcomes"):
        op.create_table(
            ...
            ),  # Users might not have rigorous FK constraints in this phase or we might use ForeignKey
        ...
        op.create_index(
            "ix_purchase_outcomes_service", "purchase_outcomes", ["service"]
        )
        op.create_index(
            "ix_purchase_outcomes_created_at", "purchase_outcomes", ["created_at"]
        )

        # Composite indexes
        op.create_index(
            "ix_po_svc_assigned_date",
            "purchase_outcomes",
            ["service", "assigned_code", "created_at"],
        )
        op.create_index(
            "ix_po_svc_requested_date",
            "purchase_outcomes",
            ["service", "requested_code", "created_at"],
        )
        op.create_index(
            "ix_po_carrier_svc", "purchase_outcomes", ["assigned_carrier", "service"]
        )
