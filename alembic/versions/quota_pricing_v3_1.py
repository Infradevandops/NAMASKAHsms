...
def upgrade():
    ...
    if "tiers" not in existing_tables:
        ...
        if "overage_rate" not in existing_cols:
            op.add_column(
                "tiers", sa.Column("overage_rate", sa.DECIMAL(10, 2), nullable=True)
            )
    ...
    if "users" in existing_tables:
        ...
        if "quota_balance" not in user_cols:
            op.add_column(
                "users",
                sa.Column("quota_balance", sa.DECIMAL(10, 2), server_default="0.00"),
            )
        if "quota_reset_date" not in user_cols:
            op.add_column(
                "users", sa.Column("quota_reset_date", sa.TIMESTAMP, nullable=True)
            )
        if "total_overage_charges" not in user_cols:
            op.add_column(
                "users",
                sa.Column(
                    "total_overage_charges", sa.DECIMAL(10, 2), server_default="0.00"
                ),
            )
    ...
    if "quota_transactions" not in existing_tables:
        ...
        op.create_index(
            "idx_quota_transactions_user", "quota_transactions", ["user_id"]
        )
        op.create_index(
    ...
    for tier, price, quota, rate, api_limit, features in [
        (
            ...
            2.20,
            10,
            '["area_code_selection", "isp_filtering", "api_access"]',
        ),
        (
            "pro",
            49.00,
            ...
            2.10,
            None,
            '["area_code_selection", "isp_filtering", "api_access", "priority_support"]',
        ),
