...
def upgrade():
    ...
    if "quota_transactions" not in existing_tables:
        op.create_table(
            ...
            sa.Column("verification_id", sa.dialects.postgresql.UUID(as_uuid=True) if bind.dialect.name == 'postgresql' else sa.UUID(), nullable=True),
    ...
    connection.execute(sa.text("""
        ...
        VALUES ('turbo', 25.00, 30.00, 2.20, 10,
                '["area_code_selection", "isp_filtering", "api_access"]'::jsonb, CURRENT_TIMESTAMP)
        ON CONFLICT (tier_name) DO UPDATE SET
            monthly_price = 25.00, included_quota = 30.00, overage_rate = 2.20, api_keys_limit = 10
    """))

    connection.execute(sa.text("""
        INSERT INTO tiers (tier_name, monthly_price, included_quota, overage_rate, api_keys_limit, features, created_at)
        VALUES ('pro', 49.00, 65.00, 2.10, NULL,
                '["area_code_selection", "isp_filtering", "api_access", "priority_support"]'::jsonb, CURRENT_TIMESTAMP)
        ON CONFLICT (tier_name) DO UPDATE SET
            monthly_price = 49.00, included_quota = 65.00, overage_rate = 2.10, api_keys_limit = NULL
    """))

    connection.execute(sa.text("""
        INSERT INTO tiers (tier_name, monthly_price, included_quota, overage_rate, api_keys_limit, features, created_at)
        VALUES ('custom', NULL, NULL, 2.00, NULL, '["all"]'::jsonb, CURRENT_TIMESTAMP)
