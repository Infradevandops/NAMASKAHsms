    ...
    def get_missing_indexes(self) -> list:
        ...
        try:
            ...
            for table_name, required_indexes in tables_to_check.items():
                ...
                existing_columns = {idx['column_names'][0] for idx in existing_indexes if len(idx['column_names']) == 1}
        ...
        for table, table_stats in stats.items():
            if isinstance(table_stats, dict) and 'row_count' in table_stats:
                ...
                if row_count > 10000 and index_count < 3:
                    recommendations.append(f"Table '{table}' has {row_count} rows but only {index_count} indexes. Consider adding more indexes for frequently queried columns.")
                
                if row_count > 100000:
                    recommendations.append(f"Table '{table}' has {row_count} rows. Consider implementing data archival strategy.")
        
        results['recommendations'] = recommendations
        
        logger.info(f"Database optimization completed. Created {len(results['indexes']['created'])} indexes.")
        return results
