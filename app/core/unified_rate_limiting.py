...
class UnifiedRateLimiter:
    ...
    def __init__(self):
        ...
        self.public_paths = [
            ...
            "/admin", "/docs", "/redoc", "/openapi.json", "/system/health",
            "/api/health", "/static", "/api/diagnostics",
        ]
    ...
    def check_token_bucket_limit(self, user_id: Optional[str], ip: str) -> Tuple[bool, int]:
        ...
        if ip not in self.ip_buckets:
            self.ip_buckets[ip] = TokenBucket(capacity=20, refill_rate=2.0)
        ...
        if user_id:
            if user_id not in self.user_buckets:
                self.user_buckets[user_id] = TokenBucket(capacity=50, refill_rate=5.0)
    ...
    def check_sliding_window_limit(
        ...
        if user_id:
            ...
            user_limit = int(config.requests * 5)
    ...
    async def check_rate_limit(
        ...
        async with self._lock:
            ...
            if system_load > 0.8:
                adjusted_limit = max(1, int(config.requests * 0.2))
