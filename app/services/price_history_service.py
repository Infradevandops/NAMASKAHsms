...
class PriceHistoryService:
    ...
    async def record_current_prices(self, source: str = "textverified"):
        ...
        try:
            live_data = await self.provider_price_service.get_live_prices(
                force_refresh=True
            )
    ...
    async def _check_price_change_alert(
        self, current_price: Dict[str, Any], source: str
    ):
        ...
        if last_snapshot:
            ...
            if old_cost > 0:
                ...
                if change_pct >= 10.0:
                    logger.warning(
                        f"Significant price increase detected for {current_price['service_name']}: {change_pct:.2f}%"
                    )
    ...
    def get_price_history(
        self, service_id: str, days: int = 30
    ) -> List[Dict[str, Any]]:
