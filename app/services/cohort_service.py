...
class CohortRetentionService:
    ...
    async def get_90d_retention_data(self) -> Dict[str, Any]:
        ...
        for u in users:
            # truncate to week
            cohort_week = u.created_at.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=u.created_at.weekday())
        ...
        for act in activities:
            ...
            if 0 <= week_offset < 13:
                ...
                if "active_users_per_week" not in cohorts[week_str]:
                    cohorts[week_str]["active_users_per_week"] = [set() for _ in range(13)]
        ...
        for week, data in sorted(cohorts.items()):
            ...
            for i in range(13):
                ...
                percentage = round((count / data["size"] * 100), 2) if data["size"] > 0 else 0
