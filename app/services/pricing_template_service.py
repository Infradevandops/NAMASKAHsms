...
from app.models.pricing_template import PricingTemplate, TierPricing, PricingHistory, UserPricingAssignment
...
class PricingTemplateService:
    ...
    def get_active_template(self, region: str = "US", user_id: int = None) -> Optional[PricingTemplate]:
        ...
        if user_id:
            assignment = self.db.query(UserPricingAssignment).filter(
                UserPricingAssignment.user_id == user_id
            ).first()
            
            if assignment:
                return self.db.query(PricingTemplate).filter(
                    PricingTemplate.id == assignment.template_id
                ).first()
    ...
    def create_template(self, name: str, description: str, region: str, currency: str,
                       tiers: List[dict], admin_user_id: int, **kwargs) -> PricingTemplate:
        """Create new pricing template"""
        
        # Check if name already exists
        existing = self.db.query(PricingTemplate).filter(
    ...
    def delete_template(self, template_id: int, admin_user_id: int) -> bool:
        ...
        assigned_users = self.db.query(UserPricingAssignment).filter(
            UserPricingAssignment.template_id == template_id
        ).count()
    ...
    def rollback_to_previous(self, admin_user_id: int, region: str = "US") -> PricingTemplate:
        ...
        last_deactivation = self.db.query(PricingHistory).join(
            PricingTemplate, PricingHistory.template_id == PricingTemplate.id
        ).filter(
            and_(
                PricingHistory.action == "deactivated",
                PricingTemplate.region == region
            )
        ).order_by(PricingHistory.changed_at.desc()).first()
    ...
    def _log_history(self, template_id: int, action: str, changed_by: int,
                     previous_template_id: int = None, notes: str = None):
