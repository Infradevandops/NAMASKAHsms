...
class TargetTrackingService:
    ...
    async def get_active_target(self, month_str: Optional[str] = None) -> MonthlyTarget:
        ...
        query = select(MonthlyTarget).where(
            MonthlyTarget.month == month_str, MonthlyTarget.is_active == True
        )
    ...
    async def record_daily_snapshot(self) -> DailyUserSnapshot:
        ...
        existing_query = select(DailyUserSnapshot).where(
        ...
        # New users today
        midnight = datetime.combine(today, datetime.min.time()).replace(
            tzinfo=timezone.utc
        )
        new_users = (
            self.db.query(func.count(User.id))
            .filter(User.created_at >= midnight)
            .scalar()
        )
        ...
        def tier_count(tier_name):
            return (
                self.db.query(func.count(User.id))
                .filter(User.subscription_tier == tier_name)
                .scalar()
            )
    ...
    async def get_growth_projections(self) -> Dict[str, Any]:
        ...
        snapshots = (
            self.db.query(DailyUserSnapshot)
            .order_by(desc(DailyUserSnapshot.snapshot_date))
            .limit(30)
            .all()
        )
        ...
        if daily_velocity > 0:
            ...
            projected_date = (
                datetime.now(timezone.utc) + timedelta(days=days_to_target)
            ).strftime("%Y-%m-%d")
        ...
        return {
            ...
            "percentage": (
                round((current_users / target_count * 100), 2)
                if target_count > 0
                else 0
            ),
