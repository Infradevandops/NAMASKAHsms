...
class NotificationPreference(BaseModel):
    ...
    )  # verification, payment, system, etc.
    enabled = Column(
        Boolean, default=True, nullable=False
    )  # Master toggle for this type
    delivery_methods = Column(
        String(255), default="toast"
    )  # Comma-separated: toast,email,sms,webhook,push
    ...
    created_at_override = Column(
        Boolean, default=False
    )  # Allow override of quiet hours
    push_enabled = Column(
        Boolean, default=True, nullable=False
    ...
    def to_dict(self):
        ...
        return {
            ...
            "delivery_methods": (
                self.delivery_methods.split(",") if self.delivery_methods else []
            ),
            "quiet_hours_start": (
