...
class CarrierAnalytics(BaseModel):
    ...
    requested_carrier = Column(
        String, nullable=False
    )  # What user asked for (e.g., "verizon")
    sent_to_textverified = Column(
        String, nullable=False
    )  # Normalized value sent to API
    textverified_response = Column(
        String
    )  # What TextVerified returned (e.g., "Mobile")
