...
async def confirm_crypto_payment(
    ...
    intent = db.query(PaymentLog).filter(
        PaymentLog.reference == request.intent_id,
    ...
    if intent.state != "pending":
        return {"status": "error", "message": f"Intent is already in {intent.state} state"}

    intent.state = "processing" # 'processing' acts as review_pending
    intent.error_message = f"{intent.error_message} | Hash: {request.transaction_hash or 'None'}"
    
    # Update the associated transaction record
    tx = db.query(Transaction).filter(Transaction.reference == request.intent_id).first()
    if tx:
...
async def get_wallet_stats(
    ...
    pending_deposits = db.query(func.count(PaymentLog.id)).filter(
        PaymentLog.user_id == user_id,
