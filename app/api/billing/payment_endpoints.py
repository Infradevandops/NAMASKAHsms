    ...
    try:
        ...
        if idempotency_key:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Invalid idempotency key format (must be UUID)",
                )
...
async def verify_payment(
    ...
    try:
        ...
        if (
            ...
            if amount > 0:
                success = payment_service.credit_user(
                    user_id, amount, payment_request.reference
                )
                if success:
                    user = db.query(User).filter(User.id == user_id).first()
...
async def get_payment_methods(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve payment methods"
        )
