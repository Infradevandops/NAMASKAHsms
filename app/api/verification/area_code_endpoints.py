...
from app.services.area_code_geo import NANPA_DATA, filter_supported, get_nearby
...
async def check_area_code_availability(
    ...
    if area_code not in supported_strings:
        raise HTTPException(
            status_code=400, detail="Area code not supported by provider"
        )
    ...
    try:
        ...
        if status == "available":
            response["message"] = (
                f"{area_code} is available for {service} based on recent activity."
            )

        elif status == "unknown":
            response["message"] = (
                f"No availability data yet for {area_code} + {service}. You can proceed \u2014 if unavailable, you won't be charged."
            )
            # Show unknowns their alternatives as well just in case they decide to switch.
            response["alternatives"] = await _build_alternatives(
                db, service, area_code, supported_strings
            )

        elif status == "unavailable":
            response["message"] = (
                f"{area_code} is not available for {service}. Select from nearby area codes."
            )
            response["alternatives"] = await _build_alternatives(
                db, service, area_code, supported_strings
            )
...
async def _build_alternatives(
    db: Session, service: str, target_code: str, supported_strings: list
) -> list:
    ...
    for alt in supported_nearby:
        alt_score = await PurchaseIntelligenceService.score_availability(
            service, alt["area_code"]
        )
