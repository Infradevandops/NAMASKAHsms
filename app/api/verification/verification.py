...
async def create_verification(
    ...
    try:
        logger.info("Creating verification", extra={
            "user_id": sanitize_user_id(user_id),
            "service_name": sanitize_service_name(verification_data.service_name)
        })
        # Validate input
        logger.info("Validating service_name", extra={
            "service_name": sanitize_service_name(verification_data.service_name)
        })
    ...
    if verification.status == "pending" and verification.verification_code:
        try:
            ...
            if sms_code:
                ...
                db.commit()
                logger.info("SMS code received", extra={
                    "verification_id": sanitize_user_id(verification_id),
                    "has_code": bool(sms_code)
                })
...
async def export_verification_history(
    ...
    try:
        ...
        # Generate filename
        filename = f"namaskah_verifications_{utc_now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    try:
        ...
        for v in verifications:
            history.append({
                ...
                "code": getattr(v, 'sms_code', None)
                or getattr(v, 'verification_code', None),
                "service": v.service_name,
                "country": v.country,
