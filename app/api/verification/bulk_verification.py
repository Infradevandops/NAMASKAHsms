...
def calculate_bulk_discount(quantity: int, base_price: float) -> tuple[float,
                                                                       float, float]:
    """
...
async def get_bulk_status(
    ...
    try:
        ...
        bulk_verifications = [v for v in verifications if hasattr(v, "bulk_id")
                              and v.bulk_id == bulk_id]
        ...
        failed = sum(1 for v in bulk_verifications if v.status in ["failed",
                                                                   "timeout", "cancelled"])
...
async def cancel_bulk_verification(
    ...
    try:
        ...
        bulk_verifications = [
            v for v in verifications if hasattr(v, "bulk_id")
            and v.bulk_id == bulk_id
