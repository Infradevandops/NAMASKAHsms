...
router = APIRouter(prefix="/api/bulk-purchase", tags=["Bulk Purchase"])
...
async def create_bulk_purchase(
    ...
    """
    logger.info(f"Bulk purchase requested by user_id: {user_id}, service: {request.service}, quantity: {request.quantity}")
