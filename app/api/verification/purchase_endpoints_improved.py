...
async def request_verification_improved(
    ...
    if user.credits < sms_cost:
        logger.warning(
            f"Insufficient credits: User={user_id}, Available=${user.credits:.2f}, Required=${sms_cost:.2f}"
        )
        raise HTTPException(
    ...
    try:
        logger.info(
            f"Calling TextVerified API: service={request.service}, country={request.country}, user={user_id}"
        )
        ...
        logger.info(
            f"TextVerified success: phone={textverified_result['phone_number']}, id={textverified_result['id']}"
        )
        ...
        logger.info(
            f"Credits deducted: User={user_id}, Amount=${sms_cost:.2f}, Balance: ${old_balance:.2f} → ${new_balance:.2f}"
        )
