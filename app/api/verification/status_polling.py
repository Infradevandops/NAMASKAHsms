...
router = APIRouter(prefix="/api/verification", tags=["verification-status"])
...
class VerificationStatusService:
            ...
            if verification.activation_id:
                try:
                    ...
                    if old_status != verification.status:
                        logger.info(f"Verification {verification_id} status changed: {old_status} -> {verification.status}")
    ...
    async def poll_user_verifications(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            pending_verifications = self.db.query(Verification).filter(
