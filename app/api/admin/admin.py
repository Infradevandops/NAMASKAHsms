...
def get_platform_stats(
    ...
    try:
        ...
        # Total verifications
        total_verifications = (
            db.execute(text("SELECT COUNT(*) FROM verifications")).scalar() or 0
        )
        ...
        popular_services = [
            {"name": row[0], "count": row[1]} for row in popular_services_raw
        ]
        ...
        daily_usage = [
            {"date": str(row[0]), "count": row[1]} for row in daily_usage_raw
        ]
...
async def admin_cancel_verification(
    ...
    verification.status = (
        "failed"  # Changed from cancelled to match status system failures
    )
    db.commit()
