...
async def get_pending_settlements(
    ...
    intents = db.query(PaymentLog).filter(
    ...
    return [
        {
            ...
            "created_at": i.processing_started_at.isoformat() if i.processing_started_at else None
        }
        for i in intents
    ]
...
async def approve_settlement(
    ...
    if success:
        ...
        db.commit()
        return {"status": "success", "message": f"Settled ${intent.amount_usd} to {user.email}"}
