...
router = APIRouter(prefix="/api/admin/verifications", tags=["admin-analytics"])
...
async def get_verification_stats(
    ...
    ).join(User).group_by(Verification.user_id, User.email).order_by(desc("count")).limit(10).all()
    
    # Top services - optimized
    top_services = db.query(
        Verification.service_name,
        func.count(Verification.id).label("count")
    ).group_by(Verification.service_name).order_by(desc("count")).limit(10).all()
    ...
    return {
        ...
        "top_users": [{"user_id": u, "email": e, "verification_count": c, "total_spent": float(s or 0)} for u, e, c, s in top_users],
...
async def get_user_verifications(
    ...
    verifications = db.query(Verification).filter(
        Verification.user_id == user_id_param
