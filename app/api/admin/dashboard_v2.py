...
async def get_dashboard_v2_stats(
    ...
    try:
        ...
        provider_stats = db.query(
            ...
            func.sum(func.case([(PurchaseOutcome.sms_received == True, 1)], else_=0)).label("success"),
            func.avg(PurchaseOutcome.latency_seconds).label("avg_latency")
        ).group_by(PurchaseOutcome.provider).all()
        ...
        recent_verifications = db.query(Verification).order_by(Verification.created_at.desc()).limit(5).all()
        activities = [
            {
                "type": "verification",
                "user_id": v.user_id,
        ...
        raise HTTPException(status_code=500, detail="Failed to aggregate institutional stats")
