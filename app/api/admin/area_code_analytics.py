...
async def get_area_code_analytics(
    ...
    worst_performing = sorted([r for r in results if r["requests"] >= 5], key=lambda x: x["success_rate"])[:10]
    
    return {
        "period": f"{days}d",
        "total_purchases": total_purchases,
        "match_rate": round(matched_purchases / total_purchases, 2) if total_purchases > 0 else 0.0,
        "top_requested": top_requested,
        "worst_performing": worst_performing,
        "data_coverage": round(len(total_area_codes_known) / 350, 2)  # Rough approximation of total US area codes
    }
...
async def get_carrier_analytics(
    ...
    for c, stats in carrier_stats.items():
        if c == "unknown" and stats["count"] < 5:
            continue
        sr = stats["sms_success"] / stats["sms_total"] if stats["sms_total"] > 0 else 0.0
...
async def get_geography_analytics(
    ...
    for o in outcomes:
        ...
        if city_key not in city_stats:
            city_stats[city_key] = {"city": city, "state": state, "purchases": 0, "sms_success": 0, "sms_total": 0}
...
async def get_learning_progress(
    ...
    return {
        ...
        "alternative_selection_rate": round(alternative_selections / total_purchases, 2) if total_purchases > 0 else 0.0,
        "total_purchases": total_purchases,
