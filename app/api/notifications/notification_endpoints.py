...
async def mark_all_as_read(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
...
@router.delete("")
async def delete_all_notifications(user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
