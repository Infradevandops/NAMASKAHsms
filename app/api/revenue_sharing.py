...
async def get_affiliate_dashboard(
    ...
    pending_result = (
        db.query(db.func.sum(RevenueShare.commission_amount))
        .filter(
            RevenueShare.partner_id == current_user.id, RevenueShare.status == "pending"
        )
        .scalar()
    )
...
async def get_commission_tiers(db: Session = Depends(get_db)):
    ...
    tiers = (
        ...
        .order_by(CommissionTier.min_volume.asc())
        .all()
    )
...
async def request_payout(
    ...
    if payout_data.amount < min_payout:
        raise HTTPException(
            status_code=400, detail=f"Minimum payout amount is N{min_payout}"
        )
...
async def get_payout_history(
    ...
    payouts = (
        ...
        .filter(PayoutRequest.affiliate_id == current_user.id)
...
async def get_commission_history(
    ...
    commissions = (
        ...
        .filter(RevenueShare.partner_id == current_user.id)
