...
async def get_dashboard_stats(
    ...
    try:
        ...
        active_verifications = db.query(Verification).filter(
            Verification.user_id == user_id,
        ...
        # Total spent
        total_spent = db.query(func.sum(Verification.cost)).filter(
            Verification.user_id == user_id,
            Verification.created_at >= start_date
        ).scalar() or 0
        ...
        popular_service_result = db.query(
            ...
            desc(func.count(Verification.id))
        ).first()
        
        most_popular_service = popular_service_result[0] if popular_service_result else None
        ...
        for i in range(7):
            ...
            day_count = db.query(Verification).filter(
                Verification.user_id == user_id,
...
async def get_service_pricing(
    ...
    try:
        ...
        # Country multipliers
        country_multipliers = {
            "US": 1.0, "GB": 1.0, "CA": 1.1, "AU": 1.4,
            "DE": 1.0, "FR": 1.0, "IN": 0.3, "BR": 0.4
        }
