...
async def update_branding(
    ...
    try:
        result = await service.update_branding(
            config_id=config_id, branding_data=branding_data.dict(exclude_unset=True)
        )
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def preview_whitelabel(
    ...
    config = db.query(WhiteLabelConfig).filter(WhiteLabelConfig.id == config_id).first()
...
async def get_custom_css(config_id: int, db: Session = Depends(get_db)):
...
async def get_pwa_manifest(config_id: int, db: Session = Depends(get_db)):
...
async def get_domain_config(domain: str, db: Session = Depends(get_db)):
    ...
    return {"is_whitelabel": True, "config": config}
...
async def apply_template(
    ...
    try:
        result = await service.update_branding(
            config_id=config_id, branding_data=templates[template_id]
        )
