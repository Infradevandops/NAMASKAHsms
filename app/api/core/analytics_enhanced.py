...
async def get_analytics_summary(
    ...
    try:
        ...
        logger.info(
            f"Analytics summary calculated for user {current_user.id}: total={total_verifications}, successful={successful_verifications}, rate={success_rate:.2%}"
        )
        ...
        logger.error(
            f"Analytics calculation failed for user {current_user.id}: {str(e)}", exc_info=True
        )
...
async def get_real_time_stats(
    ...
    try:
        ...
        today_verifications = (
            db.query(Verification)
            .filter(and_(Verification.user_id == current_user.id, Verification.created_at >= today))
            .count()
        )
