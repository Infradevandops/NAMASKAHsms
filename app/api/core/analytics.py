...
async def get_real_time_stats(
    ...
    try:
        ...
        logger.error(f"Error fetching real-time stats for user {user_id}: {e}", exc_info=True)
...
async def get_status_updates(
    ...
    try:
        ...
        recent_verifications = db.query(Verification).filter(
            Verification.user_id == user_id,
            Verification.updated_at >= cutoff_time
        ).order_by(Verification.updated_at.desc()).limit(limit).all()
        ...
        updates = [
            {
                ...
                "status": v.status,
                "phone_number": v.phone_number[-4:] if v.phone_number else None,  # Last 4 digits only
        ...
        logger.error(f"Error fetching status updates for user {user_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_analytics_summary(
    ...
    try:
        ...
        failed = sum(1 for v in verifications if v.status in ["failed", "timeout", "cancelled", "error"])
        ...
        while current_date <= to_dt.date():
            count = sum(1 for v in verifications if v.created_at and v.created_at.date() == current_date)
        ...
        logger.error(f"Error generating analytics summary for user {user_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
