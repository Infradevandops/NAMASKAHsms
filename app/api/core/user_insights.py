...
async def get_latency_percentiles(
    ...
    try:
        ...
        logger.error(f"Error in get_latency_percentiles: {e}", exc_info=True)
...
async def get_carrier_insights(
    ...
    try:
        ...
        top_carriers = [
            ...
            for k, v in sorted(carrier_counts.items(), key=lambda x: -x[1]["count"])[
                :10
            ]
        ]
        ...
        logger.error(f"Error in get_carrier_insights: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_outcome_insights(
    ...
    try:
        ...
        records = (
            db.query(PurchaseOutcome).filter(PurchaseOutcome.user_id == user_id).all()
        )
        ...
        logger.error(f"Error in get_outcome_insights: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_notification_insights(
    ...
    try:
        ...
        total = len(records)
        delivered = sum(
            1 for r in records if r.status in ("delivered", "read", "clicked")
        )
        delivery_times = [
            r.delivery_time_ms
            for r in records
        ...
        for r in records:
            by_status[r.status or "unknown"] = (
                by_status.get(r.status or "unknown", 0) + 1
            )
        ...
        return {
            ...
            "avg_delivery_ms": (
                round(sum(delivery_times) / len(delivery_times))
                if delivery_times
                else None
            ),
            "avg_read_ms": (
        ...
        logger.error(f"Error in get_notification_insights: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_refund_insights(
    ...
    try:
        ...
        logger.error(f"Error in get_refund_insights: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
