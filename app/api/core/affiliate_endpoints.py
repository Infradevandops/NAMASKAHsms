...
async def apply_for_affiliate(
    ...
    logger.info(f"Affiliate application submitted by user_id: {user_id}, program_type: {application_data.get('program_type')}")
    try:
...
async def get_my_applications(
    ...
    applications = db.query(AffiliateApplication).filter(
        AffiliateApplication.email.isnot(None)  # Placeholder - would filter by user
    ).order_by(AffiliateApplication.created_at.desc()).limit(10).all()
