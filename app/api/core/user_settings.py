...
async def save_privacy(
    ...
    pref.profile_visibility = data.get("profile_visibility", pref.profile_visibility if pref.profile_visibility is not None else False)
    pref.analytics_tracking = data.get("analytics_tracking", pref.analytics_tracking if pref.analytics_tracking is not None else True)
    pref.data_retention = str(data.get("data_retention", pref.data_retention if pref.data_retention is not None else "90"))
...
async def delete_account(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not delete account due to existing dependencies.")
