...
async def generate_api_key(
    ...
    try:
        ...
        logger.error(f"Error in generate_api_key: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def revoke_api_key(
    ...
    try:
        ...
        logger.info(
            f"API key revocation requested by user_id: {user_id}, key_id: {key_id}"
        )
...
async def get_api_key_usage(
    ...
    try:
        ...
        logger.error(f"Error in get_api_key_usage: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
