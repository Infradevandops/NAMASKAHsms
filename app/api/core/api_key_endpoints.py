...
async def generate_api_key(
    key_create: APIKeyCreate, user_id: str = Depends(require_payg), db: Session = Depends(get_db)
):
...
async def revoke_api_key(
    key_id: str, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)
):
