...
async def check_app_health(db: Session = Depends(get_db)):
    ...
    try:
        ...
        enforcer_status = "healthy" if refund_policy_enforcer.is_running else "unhealthy"
    except Exception as e:
        logger.error(f"Refund enforcer health check failed: {e}")
        enforcer_status = "unhealthy"

    # Determine overall status
    if db_status == "unhealthy" or polling_status == "unhealthy" or enforcer_status == "unhealthy":
