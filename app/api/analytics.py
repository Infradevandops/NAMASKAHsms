...
def get_user_analytics(
    ...
    for service, count in popular_services:
        ...
        enhanced_services.append(
            ServiceUsage(
                ...
                count=count,
                success_rate=round(
                    (service_success / count * 100) if count > 0 else 0, 1
                ),
    ...
    for i in range(len(cost_trends) - 1):
        if cost_trends[i + 1].value > 0:
            change = (
                (cost_trends[i].value - cost_trends[i + 1].value)
                / cost_trends[i + 1].value
            ) * 100
    ...
    efficiency_factors = [
        ...
        min(
            1.0,
            50 / (total_spent / total_verifications if total_verifications > 0 else 50),
        ),  # Cost efficiency
