...
async def user_activity(
    ...
    try:
        ...
        logs = db.query(AuditLog).filter(AuditLog.user_id == user_id).order_by(desc(AuditLog.created_at)).limit(50).all()
        return {
            "activities": [
                {"event": l.event, "details": getattr(l, 'details', ''), "created_at": l.created_at.isoformat() if l.created_at else None}
                for l in logs
            ]
        }
    except Exception:
...
async def billing_payment_status(
    ...
    try:
        ...
        tx = db.query(Transaction).filter(Transaction.reference == reference, Transaction.user_id == user_id).first()
        if tx:
            return {"status": tx.status, "reference": reference, "amount": float(tx.amount or 0)}
...
async def billing_crypto_addresses(
    user_id: str = Depends(get_current_user_id)
):
