...
async def register_reseller(
    ...
    try:
        result = await service.create_reseller_account(
            user_id=current_user.id, tier=tier
        )
...
async def create_sub_account(
    ...
    reseller = (
        ...
        .filter(ResellerAccount.user_id == current_user.id)
        .first()
    )
...
async def update_custom_pricing(
    ...
    return {"success": True, "message": "Custom pricing updated", "rates": pricing_data}
...
async def update_reseller_tier(
    ...
    reseller = (
        db.query(ResellerAccount).filter(ResellerAccount.id == reseller_id).first()
    )
