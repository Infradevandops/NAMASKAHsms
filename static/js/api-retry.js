...
(function(window) {
            ...
            try {
                if (attempt > 0) {
                    ...
                    if (window.FrontendLogger) {
                        window.FrontendLogger.info(`Retry attempt ${attempt}/${config.maxRetries} for ${url}, waiting ${delay}ms`);
                    }
                    await sleep(delay);
                }
                ...
                if (response.status >= 500 || response.status === 408 || response.status === 429) {
                    if (attempt < config.maxRetries) {
                        if (window.FrontendLogger) {
                            window.FrontendLogger.warn(`Request failed with ${response.status}, will retry`, { url, attempt });
                        }
                ...
                lastError = error;
                if (window.FrontendLogger) {
                    window.FrontendLogger.warn(`Request failed: ${error.message}, attempt ${attempt + 1}/${config.maxRetries + 1}`, { url });
                }
     ...
     */
    function createRetryButton(retryCallback, message = 'Failed to load data') {
     ...
     * @param {string} message - Error message to display
     */
    function showRetryUI(container, retryCallback, message) {
     ...
     * @param {HTMLElement|string} options.container - Container for retry UI
    ...
    async function apiCall(options) {
        ...
        const doFetch = async () => {
            try {
                ...
                if (window.FrontendLogger) {
                    window.FrontendLogger.error(`API call failed after retries: ${url}`, { error: error.message });
                }
