...
def run_migration():
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_price_snapshots_service ON price_snapshots(service_id);"
            )
        )
        conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_price_snapshots_captured ON price_snapshots(captured_at DESC);"
            )
        )
        ...
        for col_name, col_type in new_cols:
            if col_name not in existing_cols:
                ...
                conn.execute(
                    text(
                        f"ALTER TABLE pricing_templates ADD COLUMN {col_name} {col_type};"
                    )
                )
            else:
