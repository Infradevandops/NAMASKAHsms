...
class CanaryDeployment:
    ...
    def get_current_metrics(self) -> Optional[Dict]:
        ...
        try:
            response = requests.get(
                f"{self.api_url}/api/metrics/deployment",
                timeout=10
            )
    ...
    def _calculate_average_metrics(self, metrics_list: List[Dict]) -> Dict:
        ...
        avg_metrics = {
            'error_rate': sum(m.get('error_rate', 0) for m in metrics_list) / len(metrics_list),
            'p95_latency': sum(m.get('p95_latency', 0) for m in metrics_list) / len(metrics_list),
            'success_rate': sum(m.get('success_rate', 100) for m in metrics_list) / len(metrics_list),
    ...
    def rollback(self) -> bool:
        ...
        try:
            response = requests.post(
                f"{self.api_url}/api/deployment/rollback",
                timeout=10
            )
            if response.status_code == 200:
