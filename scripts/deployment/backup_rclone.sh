#!/bin/bash
# Automated Rclone Backup for Namaskah
# Add to crontab: 0 2 * * * /path/to/backup_rclone.sh

set -e

# Configuration
PROJECT_DIR="/path/to/namaskah"
...
find backups/ -name "namaskah_backup_*.sql.gz" -mtime +7 -delete 2>&1 | tee -a "$LOG_FILE"
...
if [ -n "$SLACK_WEBHOOK_URL" ]; then
    curl -X POST "$SLACK_WEBHOOK_URL" \
        ...
        -d "{\"text\":\"✅ Namaskah backup completed: $LATEST_BACKUP\"}"
fi
