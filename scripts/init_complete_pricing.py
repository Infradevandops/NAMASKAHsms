...
try:
    ...
    result = db.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='user_quotas'"))
    ...
    db.execute(text("UPDATE users SET tier_id = 'payg' WHERE tier_id IS NULL OR tier_id = ''"))
    db.commit()
    print("✅ Set existing users to Pay-As-You-Go tier")
    
    # Verify the setup
    result = db.execute(text("SELECT tier, name, price_monthly, quota_usd, overage_rate FROM subscription_tiers ORDER BY price_monthly"))
    ...
    for tier in tiers:
        ...
        print(f"• {tier[1]} ({tier[0]}): ${price:.2f}/mo, ${tier[3]} quota, +${tier[4]:.2f} overage")
