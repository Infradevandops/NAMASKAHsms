...
class APISecurityScanner:
    ...
    def check_missing_auth(self, content: str, file_path: Path):
        ...
        for route in routes:
            ...
            if "Depends(" not in route_text and "get_current_user" not in content[route.end():route.end()+200]:
                self.add_issue(file_path, line_num, "Endpoint missing authentication", "HIGH")
    ...
    def check_sql_injection(self, content: str, file_path: Path):
        ...
        for pattern in sql_patterns:
            ...
            for match in matches:
                ...
                self.add_issue(file_path, line_num, "Potential SQL injection vulnerability", "CRITICAL")
    ...
    def check_input_validation(self, content: str, file_path: Path):
        ...
        for endpoint in endpoints:
            ...
            if ":" not in endpoint_text or "str" in endpoint_text:
                ...
                if "validate" not in next_lines.lower() and "pydantic" not in next_lines.lower():
                    self.add_issue(file_path, line_num, "Missing input validation", "MEDIUM")
    ...
    def check_rate_limiting(self, content: str, file_path: Path):
        ...
        if "@limiter.limit" not in content and "RateLimiter" not in content:
            self.add_issue(file_path, 1, "Missing rate limiting configuration", "MEDIUM")
    ...
    def check_cors_config(self, content: str, file_path: Path):
        ...
        for pattern in cors_patterns:
            ...
            for match in matches:
                line_num = content[:match.start()].count('\n') + 1
                self.add_issue(file_path, line_num, "Insecure CORS configuration", "HIGH")
