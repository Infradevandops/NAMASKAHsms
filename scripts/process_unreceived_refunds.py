...
def find_unreceived_verifications(db, days_back=7, min_age_minutes=10):
    ...
    unreceived = db.query(Verification).filter(
        and_(
            Verification.cost > 0,
...
def process_refund(db, verification, dry_run=True):
    ...
    try:
        ...
        logger.info(f"{'[DRY RUN] ' if dry_run else ''}Refunding ${refund_amount:.2f} to user {user.email}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process refunds for unreceived verifications')
    parser.add_argument('--days', type=int, default=7, help='Days to look back (default: 7)')
    parser.add_argument('--execute', action='store_true', help='Actually process refunds (default: dry run)')
    parser.add_argument('--min-age', type=int, default=10, help='Min age in minutes for pending (default: 10)')
    ...
    try:
        ...
        for service, data in report['by_service'].items():
            logger.info(f"    {service}: {data['count']} verifications, ${data['amount']:.2f}")
        
        logger.info(f"\n  By Status:")
        for status, data in report['by_status'].items():
            logger.info(f"    {status}: {data['count']} verifications, ${data['amount']:.2f}")
