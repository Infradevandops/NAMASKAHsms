...
def analyze_removed_files_impact():
    ...
    for removed_file in removed_files:
        module_name = removed_file.replace('app/', '').replace('.py', '').replace('/', '.')
...
def check_database_dependencies():
    ...
    try:
        ...
        service_tables = [t for t in tables if any(s in t.lower() for s in ['oauth', 'textverified_', 'pricing_service'])]
