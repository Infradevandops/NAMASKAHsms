...
def add_indexes():
    ...
    with engine.connect() as conn:
        ...
        try:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_verifications_user_id ON verifications(user_id)"))
            print("✅ Created idx_verifications_user_id")
        except Exception as e:
            print(f"⚠️  idx_verifications_user_id: {e}")
        
        try:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_verifications_status ON verifications(status)"))
            print("✅ Created idx_verifications_status")
        except Exception as e:
            print(f"⚠️  idx_verifications_status: {e}")
        
        try:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_verifications_created_at ON verifications(created_at DESC)"))
            print("✅ Created idx_verifications_created_at")
        except Exception as e:
            print(f"⚠️  idx_verifications_created_at: {e}")
        
        try:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_verifications_user_status ON verifications(user_id, status)"))
        ...
        try:
            conn.execute(text("CREATE INDEX IF NOT EXISTS idx_transactions_created_at ON transactions(created_at DESC)"))
