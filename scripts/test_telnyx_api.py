...
class TelnyxTester:
    ...
    async def test_international_area_code(self, country: str = "GB", prefix: str = "20") -> Dict[str, Any]:
        """Test 2: Can we search international numbers by area code/prefix?"""
        print(f"\n{'='*60}")
        print(f"TEST 2: International Area Code Search (Country: {country}, Prefix: {prefix})")
        print(f"{'='*60}")
    ...
    async def test_carrier_lookup(self, phone_number: str = "+14155551234") -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  Mobile Network Code: {carrier_data.get('mobile_network_code', 'N/A')}")
                print(f"  Mobile Country Code: {carrier_data.get('mobile_country_code', 'N/A')}")
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        results["tests"]["intl_area_code_london"] = await self.test_international_area_code("GB", "20")
        await asyncio.sleep(1)
        
        # Test 3: International Area Code (Berlin)
        results["tests"]["intl_area_code_berlin"] = await self.test_international_area_code("DE", "30")
        await asyncio.sleep(1)
        
        # Test 4: Carrier Lookup
        results["tests"]["carrier_lookup"] = await self.test_carrier_lookup("+14155551234")
        await asyncio.sleep(1)
        ...
        # Calculate score
        score = 0
        if results["tests"]["us_area_code"].get("supported"): score += 3
        if results["tests"]["intl_area_code_london"].get("supported"): score += 3
        if results["tests"]["carrier_lookup"].get("supported"): score += 2
        if results["tests"]["messaging_profiles"].get("supported"): score += 2
        ...
        print(f"\nFeature Support:")
        print(f"  US Area Code Filtering: {'✅' if results['tests']['us_area_code'].get('supported') else '❌'}")
        print(f"  International Area Code (London): {'✅' if results['tests']['intl_area_code_london'].get('supported') else '❌'}")
        print(f"  International Area Code (Berlin): {'✅' if results['tests']['intl_area_code_berlin'].get('supported') else '❌'}")
        print(f"  Carrier Lookup: {'✅' if results['tests']['carrier_lookup'].get('supported') else '❌'}")
        print(f"  Messaging Profiles: {'✅' if results['tests']['messaging_profiles'].get('supported') else '❌'}")
