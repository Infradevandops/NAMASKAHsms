...
def verify_user_credentials():
    ...
    for user in users:
        ...
        tier_config = db.execute(text("""
            SELECT name, price_monthly, quota_usd, has_api_access, 
                   has_area_code_selection, has_isp_filtering, api_key_limit
            FROM subscription_tiers 
            WHERE tier = :tier
        """), {"tier": sub_tier}).fetchone()
        
        if tier_config:
            tier_name, price, quota, api_access, area_codes, isp_filter, api_limit = tier_config
