...
def verify_analytics_endpoint():
    ...
    if response.status_code == 401:
        ...
        data = response.json()
        required_fields = ['total_verifications', 'successful_verifications', 'revenue', 'success_rate']
...
def verify_activity_endpoint():
    ...
    if response.status_code == 401:
        ...
        if isinstance(data, list):
            ...
            if len(data) > 0:
                activity = data[0]
                required_fields = ['service_name', 'phone_number', 'created_at', 'status']
...
def verify_error_handling():
    ...
    checks = [
        ("loadTierInfo has try-catch", "try {" in dashboard_content and "catch (error)" in dashboard_content),
        ("loadAnalytics has try-catch", "async function loadAnalytics()" in dashboard_content),
        ("loadActivity has try-catch", "async function loadActivity()" in dashboard_content),
...
def verify_settings_error_handling():
    ...
    checks = [
        ("loadUserData has try-catch", "async function loadUserData()" in settings_content),
        ("loadBillingPlans has try-catch", "async function loadBillingPlans()" in settings_content),
        ("loadApiKeys has try-catch", "async function loadApiKeys()" in settings_content),
        ("Error messages shown", "Failed to load" in settings_content or "Error loading" in settings_content),
    ]
...
def verify_loading_spinners():
    ...
    checks = [
        ...
        ("Spinner used in activity loading", "id=\"activity-loading\"" in dashboard_content),
    ]
