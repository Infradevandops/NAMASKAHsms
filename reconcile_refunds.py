...
def main():
    parser = argparse.ArgumentParser(description="Refund reconciliation for failed verifications")
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        print("=" * 80)
        print(f"Mode: {'DRY RUN (no changes)' if args.dry_run else 'EXECUTE (processing refunds)'}")
        ...
        if args.user_id:
            ...
            for v in verifications:
                ...
                print(f"{v.id} | {v.service_name} | ${v.cost:.2f} | {v.status} | {status}")
            ...
            if not args.dry_run and total_refund > 0:
                confirm = input(f"Process ${total_refund:.2f} refund for user {user.email}? (yes/no): ")
                if confirm.lower() == "yes":
                    for v in verifications:
                        result = refund_service.process_verification_refund(v.id, v.status)
                        if result:
                            print(f"✓ Refunded ${result['refund_amount']:.2f} for {v.id}")
            ...
            if args.dry_run and report['needs_refund'] > 0:
                ...
                print(f"Total amount: ${sum(v['cost'] for v in report['verifications'] if not v.get('refunded')):.2f}")
                print("=" * 80)
