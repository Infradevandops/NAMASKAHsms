...
def test_get_notifications(auth_token):
    ...
    response = client.get(
        "/api/notifications", headers={"Authorization": f"Bearer {auth_token}"}
    )
...
def test_get_referral_stats(auth_token):
    ...
    response = client.get(
        "/api/referrals/stats", headers={"Authorization": f"Bearer {auth_token}"}
    )
    assert response.status_code == 200
...
def test_get_current_tier(auth_token):
    ...
    response = client.get(
        "/api/tiers/current", headers={"Authorization": f"Bearer {auth_token}"}
    )
    assert response.status_code == 200
    data = response.json()
