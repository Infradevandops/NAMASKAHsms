...
class TestDatabaseIntegration:
    ...
    def test_transaction_user_relationship(self, db_session, regular_user):
        ...
        txs = db_session.query(Transaction).filter(
            Transaction.user_id == regular_user.id
        ).all()
    ...
    def test_payment_log_creation_and_query(self, db_session, regular_user):
        ...
        # Query
        found = db_session.query(PaymentLog).filter(
            PaymentLog.reference == "test_ref_123"
        ).first()
    ...
    def test_query_filtering_and_ordering(self, db_session, regular_user):
        ...
        credits = db_session.query(Transaction).filter(
            Transaction.user_id == regular_user.id,
    ...
    def test_cascade_delete_behavior(self, db_session):
        ...
        tx_exists = db_session.query(Transaction).filter(
    ...
    def test_query_pagination(self, db_session, regular_user):
        ...
        # Paginate
        page_1 = db_session.query(Transaction).filter(
            Transaction.user_id == regular_user.id
        ).limit(10).offset(0).all()
        
        page_2 = db_session.query(Transaction).filter(
            Transaction.user_id == regular_user.id
