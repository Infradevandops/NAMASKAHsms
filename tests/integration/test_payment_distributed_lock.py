...
class TestDistributedLock:
    ...
    async def test_credit_user_with_lock_acquires_lock(self, db, test_user, cache):
        ...
        result = await service.credit_user_with_lock(
            test_user.id, 10.0, "ref_lock_test"
        )
    ...
    @pytest.mark.asyncio
    async def test_credit_user_with_lock_prevents_concurrent_access(
        self, db, test_user, cache
    ):
        ...
        async def credit_with_delay():
            result = await service.credit_user_with_lock(
                test_user.id, 10.0, "ref_concurrent"
            )
        ...
        results = await asyncio.gather(
            credit_with_delay(), credit_with_delay(), return_exceptions=True
        )
    ...
    async def test_credit_user_with_lock_releases_on_error(self, db, cache):
        ...
        result = await service.credit_user_with_lock.__wrapped__(
            service, "nonexistent", 10.0, "ref_error_2"
        )
