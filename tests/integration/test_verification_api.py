...
def test_get_verification_history(auth_token):
    ...
    response = client.get(
        "/api/v1/verify/history", headers={"Authorization": f"Bearer {auth_token}"}
    )
...
def test_get_services(auth_token):
    ...
    response = client.get(
        "/api/services", headers={"Authorization": f"Bearer {auth_token}"}
    )
    assert response.status_code == 200
...
def test_get_countries(auth_token):
    ...
    response = client.get(
        "/api/countries", headers={"Authorization": f"Bearer {auth_token}"}
    )
    assert response.status_code == 200
