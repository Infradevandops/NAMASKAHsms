...
class TestAdminWorkflow:
    ...
    def test_user_management_workflow(self, client, admin_user, regular_user):
        ...
        # 1. List users
        with patch(
            "app.core.dependencies.get_current_user_id", return_value=admin_user.id
        ):
...
class TestAPIKeyWorkflow:
    ...
    def test_api_key_lifecycle(self, client, payg_user, db):
        ...
        # 1. Create API key
        with patch(
            "app.core.dependencies.get_current_user_id", return_value=payg_user.id
        ):
            with patch("app.core.dependencies.require_tier", return_value=payg_user.id):
                create_response = client.post(
                    "/api/v1/auth/api-keys", json={"name": "Test Key"}
                )
