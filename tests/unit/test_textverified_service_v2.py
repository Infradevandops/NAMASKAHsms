...
async def test_build_area_code_preference_chain():
    ...
    with patch.object(service, "get_area_codes_list", new_callable=AsyncMock) as mock_get:
...
async def test_create_verification_with_fallback_tracking():
    ...
    with patch.object(service, "_build_area_code_preference", new_callable=AsyncMock) as mock_pref:
        mock_pref.return_value = ["212", "718", "917"]
        
        with patch.object(service, "_get_area_codes_by_state", new_callable=AsyncMock) as mock_by_state:
