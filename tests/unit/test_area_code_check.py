...
def mock_cache_and_tv(db_session):
    ...
    with patch(
        "app.api.verification.area_code_endpoints.cache.get", new_callable=AsyncMock
    ) as m_get, patch(
        "app.api.verification.area_code_endpoints.cache.set", new_callable=AsyncMock
    ), patch(
        "app.services.purchase_intelligence.cache.get", new_callable=AsyncMock
    ) as pi_get, patch(
        "app.services.purchase_intelligence.cache.set", new_callable=AsyncMock
    ), patch(
        "app.api.verification.area_code_endpoints._tv.get_area_codes_list",
...
def _patch_session(db_session):
    ...
    return patch(
        "app.services.purchase_intelligence.SessionLocal", return_value=db_session
    )
...
async def test_area_code_check_unsupported(authenticated_client):
    ...
    response = authenticated_client.get(
        "/api/area-codes/check?service=whatsapp&area_code=999"
    )
...
async def test_area_code_check_unknown_to_tv(authenticated_client, mock_cache_and_tv):
    ...
    response = authenticated_client.get(
        "/api/area-codes/check?service=whatsapp&area_code=212"
    )
    assert response.status_code == 400
...
@pytest.mark.asyncio
async def test_area_code_check_available(
    authenticated_client, db_session, mock_cache_and_tv
):
    ...
    with _patch_session(db_session):
        response = authenticated_client.get(
            "/api/area-codes/check?service=whatsapp&area_code=213"
        )
...
@pytest.mark.asyncio
async def test_area_code_check_unavailable_with_alternatives(
    authenticated_client, db_session, mock_cache_and_tv
):
