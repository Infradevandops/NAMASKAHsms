...
class TestRetryNotifications:
    ...
    async def test_notify_retry_attempt_called(self):
        ...
        with patch.object(dispatcher.notification_service, 'create_notification') as mock_create:
    ...
    async def test_retry_notification_final_attempt(self):
        ...
        with patch.object(dispatcher.notification_service, 'create_notification') as mock_create:
            ...
            call_args = mock_create.call_args[1]
            assert "final" in call_args["message"].lower() or "final" in call_args["title"].lower()
...
class TestNotificationIntegration:
    ...
    async def test_notifications_dont_block_purchase(self):
        ...
        with patch.object(dispatcher.notification_service, 'create_notification', side_effect=Exception("DB error")):
