...
class TestCreditServiceComplete:
    ...
    def test_transfer_credits(self, db_session, regular_user, admin_user):
        ...
        reg_trans = db_session.query(Transaction).filter(Transaction.user_id == regular_user.id, Transaction.type == "transfer").first()
        admin_trans = db_session.query(Transaction).filter(Transaction.user_id == admin_user.id, Transaction.type == "transfer").first()
