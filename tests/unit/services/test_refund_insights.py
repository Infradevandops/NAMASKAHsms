...
async def test_handle_timeout_logs_insight():
    ...
    with patch("app.services.sms_polling_service.PurchaseIntelligenceService.update_sms_received", new_callable=AsyncMock) as mock_update_telemetry, \
         patch("app.services.sms_polling_service.refund_policy_enforcer.enforce_single_verification", new_callable=AsyncMock) as mock_enforce:
...
async def test_poll_textverified_timeout_triggers_insight():
    ...
    service.textverified.get_verification_details.return_value = {"number": "1234567890", "id": "act-456"}
    ...
    with patch.object(service, "_handle_timeout", new_callable=AsyncMock) as mock_timeout:
        ...
        with patch("app.core.database.SessionLocal", return_value=mock_db), \
             patch("app.models.Verification", return_value=mock_verification):
            ...
            await service._poll_textverified(mock_verification, mock_db, timeout_seconds=10)
