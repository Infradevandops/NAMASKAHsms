...
class TestCreditServiceTransfer:
    ...
    def test_transfer_credits_success(self, db_session):
        ...
        # Verify transactions
        txs_sender = db_session.query(Transaction).filter(Transaction.user_id == sender.id, Transaction.type == "transfer").all()
        txs_recipient = db_session.query(Transaction).filter(Transaction.user_id == recipient.id, Transaction.type == "transfer").all()
...
class TestCreditServiceAdmin:
    ...
    def test_reset_credits(self, db_session):
        ...
        tx = db_session.query(Transaction).filter(Transaction.user_id == user.id, Transaction.type == "admin_reset").first()
        assert tx is not None
