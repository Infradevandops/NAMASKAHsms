...
class TestTransactionIdempotency:
    ...
    def test_idempotency_key_unique_constraint(self, db):
        ...
        t1 = Transaction(
            user_id="user1", amount=10.0, type="credit", idempotency_key=key
        )
        db.add(t1)
        db.commit()

        t2 = Transaction(
            user_id="user2", amount=20.0, type="credit", idempotency_key=key
        )
        db.add(t2)
    ...
    def test_payment_log_linking(self, db):
        ...
        txn = Transaction(
            user_id="user1", amount=10.0, type="credit", payment_log_id=log.id
        )
        db.add(txn)
        db.commit()
