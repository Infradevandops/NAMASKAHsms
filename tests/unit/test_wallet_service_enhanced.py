...
class MockWalletService:
    ...
    def deduct_credits(
        self, user_id: str, amount: float, description: str = None
    ) -> dict:
...
class TestWalletService:
    ...
    def test_user(self, db_session):
        user = User(id="wallet_user", email="wallet@example.com", credits=100.0)
        db_session.add(user)
        db_session.commit()
        return user
    ...
    def test_add_credits_success(self, wallet_service, test_user, db_session):
        ...
        transaction = (
            ...
            .filter(Transaction.id == result["transaction_id"])
            .first()
        )
    ...
    def test_negative_balance_prevention(self, wallet_service, db_session):
        ...
        user = User(id="zero_user", email="zero@example.com", credits=5.0)
        db_session.add(user)
        db_session.commit()
