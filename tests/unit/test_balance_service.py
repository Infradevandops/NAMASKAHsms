...
async def test_get_user_balance_regular_user():
    ...
    db = MagicMock()
    user = User(
        id="user123", email="user@test.com", is_admin=False, credits=Decimal("25.50")
    )
...
async def test_get_user_balance_admin_user():
    ...
    db = MagicMock()
    user = User(
        id="admin123", email="admin@test.com", is_admin=True, credits=Decimal("10.00")
    )
    db.query().filter().first.return_value = user
...
async def test_check_sufficient_balance_insufficient():
    ...
    db = MagicMock()
    user = User(
        id="user123", email="user@test.com", is_admin=False, credits=Decimal("5.00")
    )
    db.query().filter().first.return_value = user
