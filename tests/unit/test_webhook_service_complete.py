...
class TestWebhookServiceComplete:
    ...
    def test_webhook_registration(self, db_session, regular_user):
        ...
        saved = db_session.query(Webhook).filter(
            Webhook.user_id == regular_user.id
        ).first()
