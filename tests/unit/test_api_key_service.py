...
class TestCanCreateKey:
    ...
    def test_reads_subscription_tier_not_tier(self, db, regular_user):
        ...
        db.commit()
        with patch("app.services.api_key_service.TierConfig.get_tier_config") as mock_cfg:
...
class TestGetRemainingKeys:
    ...
    def test_get_remaining_keys_pro(self, db, regular_user):
        regular_user.subscription_tier = "pro"
        db.commit()
        before = db.query(APIKey).filter(
            APIKey.user_id == regular_user.id, APIKey.is_active.is_(True)
        ).count()
        for i in range(3):
            db.add(APIKey(
...
class TestGetUserKeys:
    ...
    def test_get_user_keys_excludes_inactive_when_false(self, db, regular_user):
        ...
        uid = str(uuid.uuid4())
        db.add(APIKey(
            id=f"active-{uid}", user_id=regular_user.id, name="active",
            key_hash=f"h1-{uid}", key_preview="nsk_...", is_active=True,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
        db.add(APIKey(
            id=f"revoked-{uid}", user_id=regular_user.id, name="revoked",
            key_hash=f"h2-{uid}", key_preview="nsk_...", is_active=False,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
    ...
    def test_get_user_keys_includes_inactive_when_true(self, db, regular_user):
        ...
        uid = str(uuid.uuid4())
        db.add(APIKey(
            id=f"active2-{uid}", user_id=regular_user.id, name="active2",
            key_hash=f"h3-{uid}", key_preview="nsk_...", is_active=True,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
        db.add(APIKey(
            id=f"revoked2-{uid}", user_id=regular_user.id, name="revoked2",
            key_hash=f"h4-{uid}", key_preview="nsk_...", is_active=False,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
...
class TestValidateApiKey:
    ...
    def test_returns_none_for_expired_key(self, db, regular_user):
        ...
        uid = str(uuid.uuid4())
        db.add(APIKey(
            id=f"expired-{uid}", user_id=regular_user.id, name="expired",
            key_hash=f"expiredhash-{uid}", key_preview="nsk_...", is_active=True,
    ...
    def test_returns_key_for_valid(self, db, regular_user):
        ...
        uid = str(uuid.uuid4())
        db.add(APIKey(
            id=f"valid-{uid}", user_id=regular_user.id, name="valid",
            key_hash=f"validhash-{uid}", key_preview="nsk_...", is_active=True,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
    ...
    def test_returns_none_for_revoked_key(self, db, regular_user):
        ...
        uid = str(uuid.uuid4())
        db.add(APIKey(
            id=f"revoked3-{uid}", user_id=regular_user.id, name="revoked",
            key_hash=f"revokedhash-{uid}", key_preview="nsk_...", is_active=False,
            created_at=datetime.now(timezone.utc),
            expires_at=datetime.now(timezone.utc) + timedelta(days=365),
            request_count=0,
        ))
