...
class TestPricingCalculator:
    def test_calculate_sms_cost_freemium(self, db_session, regular_user):
        ...
        with pytest.raises(ValueError, match="Filters not available"):
            PricingCalculator.calculate_sms_cost(
                db_session, regular_user.id, filters={"isp": True}
            )
    ...
    def test_calculate_sms_cost_payg(self, db_session, regular_user):
        ...
        res = PricingCalculator.calculate_sms_cost(
            db_session, regular_user.id, filters={"state": True, "isp": True}
        )
        assert res["base_cost"] == 2.50
    ...
    def test_validate_balance(self, db_session, regular_user):
        ...
        assert (
            PricingCalculator.validate_balance(db_session, regular_user.id, 5.0) is True
        )

        regular_user.bonus_sms_balance = 0.0
        assert (
            PricingCalculator.validate_balance(db_session, regular_user.id, 5.0)
            is False
        )
        ...
        assert (
            PricingCalculator.validate_balance(db_session, regular_user.id, 15.0)
            is False
        )
