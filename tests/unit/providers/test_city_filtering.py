...
def _result(provider="textverified", phone="+12025551234", city_honoured=True, city_note=None):
...
def test_us_always_routes_textverified():
    ...
    with patch.object(router, "_get_textverified") as mock_tv:
        ...
        provider, city_attempted, _ = router.get_provider("US", city="New York", user_tier="freemium")
...
def test_international_city_pro_routes_telnyx():
    ...
    with patch.object(router, "_get_telnyx") as mock_telnyx:
        ...
        provider, city_attempted, pre_note = router.get_provider("GB", city="London", user_tier="pro")
...
def test_international_city_payg_routes_fivesim_city_dropped():
    ...
    with patch.object(router, "_get_fivesim") as mock_5sim:
        ...
        provider, city_attempted, pre_note = router.get_provider("GB", city="London", user_tier="payg")
...
def test_international_no_city_routes_fivesim():
    ...
    with patch.object(router, "_get_fivesim") as mock_5sim:
        mock_5sim.return_value.name = "5sim"
        mock_5sim.return_value.enabled = True
        provider, city_attempted, _ = router.get_provider("DE", city=None, user_tier="payg")
    assert provider.name == "5sim"
    assert city_attempted is False
...
async def test_telnyx_empty_city_retries_without_city():
    ...
    found_resp.json.return_value = {"data": [{"phone_number": "+447911123456", "cost_information": {"upfront_cost": "1.00"}}]}
...
async def test_telnyx_empty_country_failover_to_fivesim():
    ...
    mock_fivesim.purchase_number = AsyncMock(return_value=_result("5sim", "+447700900123"))

    with patch.object(router, "get_provider", return_value=(mock_telnyx, True, None)), \
         patch.object(router, "_get_failover_provider", return_value=mock_fivesim), \
         patch("app.services.providers.provider_router.get_settings") as mock_settings_fn:
        s = MagicMock()
        s.enable_provider_failover = True
        mock_settings_fn.return_value = s

        result = await router.purchase_with_failover("whatsapp", "GB", city="London", user_tier="pro")
...
def test_provider_error_user_message_has_no_provider_names():
    ...
    provider_names = ["telnyx", "5sim", "textverified", "Telnyx", "5Sim", "TextVerified"]
    ...
    for category in ProviderError.USER_MESSAGES:
        ...
        for name in provider_names:
            assert name.lower() not in msg.lower(), (
