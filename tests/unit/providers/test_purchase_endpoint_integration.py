...
def test_purchase_us_routes_textverified(auth_client, db, funded_user, test_user_id):
    ...
    with patch("app.api.verification.purchase_endpoints.ProviderRouter") as MockRouter, \
         patch("app.api.verification.purchase_endpoints.PricingCalculator") as MockPricing, \
         patch("app.api.verification.purchase_endpoints.BalanceService") as MockBalance, \
         patch("app.api.verification.purchase_endpoints.TransactionService"), \
         patch("app.api.verification.purchase_endpoints.NotificationDispatcher"), \
         patch("app.api.verification.purchase_endpoints.RefundService") as MockRefund, \
         patch("app.api.verification.purchase_endpoints.QuotaService"), \
         patch("app.api.verification.purchase_endpoints.sms_polling_service"):

        MockRouter.return_value.purchase_with_failover = AsyncMock(return_value=result)
        MockPricing.calculate_sms_cost.return_value = {"total_cost": 2.22, "carrier_surcharge": 0.0, "area_code_surcharge": 0.0}
        MockBalance.check_sufficient_balance = AsyncMock(return_value={"sufficient": True, "current_balance": 50.0, "source": "local"})
        MockRefund.return_value.process_refund = AsyncMock(return_value={"refund_issued": False})

        response = auth_client.post("/api/verification/request", json={
            "service": "whatsapp",
            "country": "US",
            "capability": "sms",
        })
    ...
    verification = db.query(Verification).filter(
        Verification.activation_id == "order-test-123"
    ).first()
...
def test_purchase_gb_routes_fivesim(auth_client, db, funded_user, test_user_id):
    ...
    with patch("app.api.verification.purchase_endpoints.ProviderRouter") as MockRouter, \
        ...
        MockRouter.return_value.purchase_with_failover = AsyncMock(return_value=result)
        MockPricing.calculate_sms_cost.return_value = {"total_cost": 2.50, "carrier_surcharge": 0.0, "area_code_surcharge": 0.0}
        MockBalance.check_sufficient_balance = AsyncMock(return_value={"sufficient": True, "current_balance": 50.0, "source": "local"})
        MockRefund.return_value.process_refund = AsyncMock(return_value={"refund_issued": False})
    ...
    verification = db.query(Verification).filter(
        Verification.phone_number == "+447911123456"
    ).first()
    assert verification is not None
...
def test_verification_record_provider_field(auth_client, db, funded_user, test_user_id):
    ...
    verification = db.query(Verification).filter(
        Verification.phone_number == "+4915123456789"
    ).first()
    assert verification is not None
