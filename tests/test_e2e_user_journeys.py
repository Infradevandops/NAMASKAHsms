...
class TestFreemiumUserJourney:
    ...
    def test_freemium_user_gets_402_on_api_access(
     ...
     headers={"Authorization": f"Bearer {token}"})
        ...
        data = response.json()
        # Check for required_tier in response (may be nested in message or at
        # top level)
        has_tier_info = (
    ...
    def test_freemium_user_can_view_dashboard(
        self, client: TestClient, db: Session):
    ...
    def test_freemium_user_sees_upgrade_options(
        self, client: TestClient, db: Session):
