...
class TestTierCurrentByTier:
    ...
    @pytest.mark.parametrize("tier,has_api_access", [
        ("freemium", False),
        ("payg", True),
        ("pro", True),
        ("custom", True),
...
class TestTierResponseValidation:
    ...
    def test_response_passes_frontend_validation(self, client: TestClient, auth_headers: dict):
        ...
        required_fields = [
            'current_tier', 'tier_name', 'price_monthly', 'quota_usd',
            'quota_used_usd', 'quota_remaining_usd', 'sms_count',
            'within_quota', 'overage_rate', 'features'
        ]
