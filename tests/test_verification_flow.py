...
try:
    ...
    required_columns = [
        'id', 'user_id', 'service_name', 'phone_number', 'country',
        'capability', 'status', 'cost', 'activation_id', 'provider',
        'idempotency_key', 'sms_code', 'sms_text', 'created_at'
    ]
...
try:
    ...
    try:
        db = SessionLocal()
        test_query = db.query(Verification).filter(
