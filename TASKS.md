...
- T2 · `auto_topup_service` rewired to use `charge_authorization` on saved card + credit user directly; `deduct_credits` now triggers `check_and_topup` after every debit
...
- T7 · Webhook idempotency verified; idempotency key wired through `POST /wallet/paystack/initialize`; race condition test passes — also fixed `IntegrityError` bug in `credit_user`
